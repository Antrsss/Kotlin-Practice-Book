...
class MainActivity : AppCompatActivity() {
    ...
    private fun launchTrackingService() {
        RouteTrackingService.trackingCompletion
            .observe(this) { agentId ->
                showResult("Agent $agentId arrived!")
        }
        val serviceIntent = Intent(
            this, RouteTrackingService::class.java
        ).apply {
    ...
    private fun dispatchCat() {
        ...
        val catStretchingRequest = OneTimeWorkRequest
            ...
            .setInputData(
                getCatAgentIdInputData(
                    CatStretchingWorker.INPUT_DATA_CAT_AGENT_ID, catAgentId
                )
            ).build()
        val catFurGroomingRequest = OneTimeWorkRequest
            ...
            .setInputData(
                getCatAgentIdInputData(
                    CatFurGroomingWorker.INPUT_DATA_CAT_AGENT_ID, catAgentId
                )
            ).build()
        val catLitterBoxSittingWorker = OneTimeWorkRequest
            .Builder(CatLitterBoxSittingWorker::class.java)
            .setConstraints(networkConstraints)
            .setInputData(
                getCatAgentIdInputData(
                    CatLitterBoxSittingWorker.INPUT_DATA_CAT_AGENT_ID, catAgentId
                )
            ).build()
        val catSuitUpWorker = OneTimeWorkRequest
            .Builder(CatSuitUpWorker::class.java)
            .setConstraints(networkConstraints)
            .setInputData(
                getCatAgentIdInputData(
                    CatSuitUpWorker.INPUT_DATA_CAT_AGENT_ID, catAgentId
                )
            ).build()
        ...
        workManager.getWorkInfoByIdLiveData(catLitterBoxSittingWorker.id)
            .observe(this) { info ->
                if (info.state.isFinished) {
                    showResult("Agent done sitting in little box")
                }
            }
        workManager.getWorkInfoByIdLiveData(catSuitUpWorker.id)
            .observe(this) { info ->
                if (info.state.isFinished) {
